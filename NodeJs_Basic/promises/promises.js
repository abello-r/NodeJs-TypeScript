...
// Class instances
	...
	file_manager_process.then((file) =>
	{
		return file;
	...
	{
		let buf = Buffer.allocUnsafe(50).fill(0);
		let r_bytes_promise = file.read(buf);		

		r_bytes_promise.then( (r_bytes) =>
		{
		...
		{
			let end_buff_promise = file.readToEnd();
	
			end_buff_promise.then( (end_buff) =>
			{
				console.log('$' + end_buff.toString() + '$');
			});
			return file;
